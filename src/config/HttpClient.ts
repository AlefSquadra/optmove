...
export class HttpClient {
  ...
  async request<T>(config: CustomAxiosRequestConfig): Promise<T> {
    const response: AxiosResponse<T> = await this.axiosInstance.request<T>(config);
    return response.data;
  }

  get<T>(url: string, config?: Partial<CustomAxiosRequestConfig>) {
    return this.request<T>({ method: "GET", url, ...config } as CustomAxiosRequestConfig);
  }

  post<T, K>(url: string, data?: K, config?: Partial<CustomAxiosRequestConfig>) {
    return this.request<T>({ method: "POST", url, data, ...config } as CustomAxiosRequestConfig);
  }

  put<T, K>(url: string, data?: K, config?: Partial<CustomAxiosRequestConfig>) {
    return this.request<T>({ method: "PUT", url, data, ...config } as CustomAxiosRequestConfig);
  }

  delete<T>(url: string, config?: Partial<CustomAxiosRequestConfig>) {
